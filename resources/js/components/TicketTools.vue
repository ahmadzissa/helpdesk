...
<template v-if="history && !historyLoading"><h3 v-if="!archiveOnly && !historySearch && history.open.length">Other open tickets</h3><div v-for="item in !archiveOnly && !historySearch ? history.open : []" :key="item.id" class="history-row"><div><Link :href="$appUrl('/tickets/' + item.id)">#{{ item.id }} · {{ item.subject }}</Link><small>{{ item.status }}{{ item.similar ? ' · Similar subject' : '' }}{{ !item.merge_allowed ? ' · Different mailbox' : '' }}</small></div></div>
<h3>{{ archiveOnly ? "Archived tickets" : "Previous conversations" }} ({{ history.history.total }})</h3><div v-for="item in history.history.data" :key="item.id" class="history-row"><div><Link :href="$appUrl('/tickets/' + item.id)">#{{ item.id }} · {{ item.subject }}</Link><small>{{ item.status }} · {{ item.folder }} · {{ when(item.created_at) }}{{ item.merged_into_id ? ' · Merged into #' + item.merged_into_id : '' }}</small></div></div><p v-if="!history.history.total" class="muted">{{ archiveOnly ? "No archived tickets match this search." : "No previous conversations match this search." }}</p>
