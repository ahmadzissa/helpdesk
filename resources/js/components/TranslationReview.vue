...
async function translate() {
    ...
    try {
        ...
        automaticSend = config.settings.outgoing && config.settings.auto_send;
...
<template v-if="preview"><strong>Reply in {{ languageName(preview.context.target) }}</strong><label>Translated reply<textarea v-model="preview.body" rows="9" :disabled="busy" /></label><button type="button" class="text-button" @click="showOriginal = !showOriginal">{{ showOriginal ? 'Hide original' : 'Show original' }}</button><pre v-if="showOriginal" class="translation-plain" dir="auto">{{ preview.originalBody }}</pre></template>
<div class="form-actions"><button v-if="error" class="secondary-button" @click="send(true)" :disabled="busy">Send original language</button><button class="secondary-button" @click="translate" :disabled="busy">{{ preview ? 'Translate again' : 'Retry translation' }}</button><button v-if="preview" class="primary-button" @click="send" :disabled="busy || !preview.body.trim() || !preview.subject.trim()">{{ busy ? 'Saving…' : state.workspace.sending_safety?.paused ? 'Save translation for review' : 'Send translated reply' }}</button></div></div></Modal>
</template>
