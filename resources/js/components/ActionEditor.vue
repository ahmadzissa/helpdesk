...
const types = [['status', 'Set status'], ['priority', 'Set priority'], ['folder', 'Move to folder'], ['assignee_id', 'Assign agent'], ['team_id', 'Assign team'], ['add_tag', 'Add tag'], ['remove_tag', 'Remove tag'], ['reply_id', 'Send canned response'], ['note', 'Add private note'], ['follow_up', 'Schedule follow-up']];
const options = type => ({
...
<div class="workflow-actions">
    <div v-for="(action, index) in actions" :key="index" class="workflow-action">
        <div class="workflow-row"><span class="step-number">{{ index + 1 }}</span><select v-model="action.type" @change="change(action)" :aria-label="'Action ' + (index + 1)"><option v-for="[value, label] in types" :key="value" :value="value">{{ label }}</option></select>
            ...
            <input v-else-if="action.type !== 'note'" v-model="action.value" :aria-label="'Value for action ' + (index + 1)" placeholder="Tag" maxlength="60" required />
        ...
        <textarea v-if="action.type === 'note'" v-model="action.value" placeholder="Private note. Variables such as {{name}} are supported." aria-label="Private note text" rows="3" required maxlength="20000" />
