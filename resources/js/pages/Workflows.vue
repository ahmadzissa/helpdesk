...
<div v-else class="workflow-history"><article v-for="entry in data[tab]" :key="entry.id" class="management-row"><Icon :name="tab === 'runs' ? 'bolt' : 'clock'" /><div class="management-row-main"><Link :href="$appUrl('/tickets/' + entry.ticket_id)">{{ entry.subject || 'Open ticket' }} · {{ entry.name || 'Timed follow-up' }}</Link><p class="muted">{{ tab === 'runs' ? when(entry.created_at) : when(entry.due_at) + ' · ' + entry.state + (entry.result ? ' · ' + entry.result : '') }}</p></div></article><p v-if="!data[tab].length" class="muted">Nothing here yet.</p><small class="muted">Most recent 50 entries.</small></div>
</div></main>
...
<style scoped>
.automation-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}
.automation-card{display:flex;flex-direction:column;min-width:0;min-height:240px}
.automation-card-heading{flex-wrap:wrap}.automation-card-heading>div{min-width:0;overflow-wrap:anywhere}
.workflow-summary{flex:1;overflow-wrap:anywhere}
.automation-card-footer{display:flex;justify-content:flex-end;padding:0 24px 20px}.automation-card-footer p{margin:0;font-size:14px}
@media(max-width:1100px){.automation-list{grid-template-columns:minmax(0,1fr)}}
