...
async function showReplyTranslation() { details.value = true; await nextTick(); replyTranslationPanel.value?.scrollIntoView({ block: 'nearest' }); }
...
async function send(sendOriginal = false) {
    ...
    sendOriginal = sendOriginal === true && !privateNote.value;
    if (sendOriginal && autoReply.value) { notify('Translate this reply into the customer language before sending.', true); return; }
    if (!sendOriginal && requiresPreview.value && !previewReady.value) await showReplyTranslation();
    ...
    try {
        ...
        if (body.value !== sendingOriginal || privateNote.value !== sendingPrivate || (useTranslation && !previewReady.value)) throw new Error('The reply changed. Review it before sending.');
...
<main v-else class="ticket-page surface">
    ...
    <aside v-if="details" class="ticket-inspector">
        ...
        <div class="inspector-scroll">
            ...
            <section class="inspector-section inspector-translation"><h3>Translation</h3><span class="reading-language"><Icon name="globe" :size="14" />Reading language: {{ languageName(translationSettings.target) }}</span><button class="text-button" @click="translateAll(true)">{{ translationErrors.all ? 'Retry email translation' : 'Translate all emails' }}</button><p v-if="translationErrors.all" class="error-message" role="alert">{{ translationErrors.all }} Originals remain available.</p></section>
...
<TranslationReview v-if="reviewMessage" :message="reviewMessage" :ticket="ticket" :detect-language="detectLanguage" @close="reviewMessage = null" @saved="reviewedTranslation" />
