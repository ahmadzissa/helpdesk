...
import InspectorSelect from '../components/InspectorSelect.vue';
...
const savingProperty = ref('');
const propertyDisabled = computed(() => sending.value || Boolean(savingProperty.value) || Boolean(ticket.value?.merged_into_id));
const statusOptions = computed(() => state.workspace.statuses.map(value => ({ value, label: value })));
const priorityOptions = computed(() => [...state.workspace.priorities].reverse().map(value => ({ value, label: value })));
const agentOptions = computed(() => [{ value: null, label: 'Unassigned', description: 'No agent assigned' }, ...state.workspace.agents.map(agent => ({ value: agent.id, label: agent.name, description: agent.email }))]);
const teamOptions = computed(() => [{ value: null, label: 'No team' }, ...state.workspace.teams.map(team => ({ value: team.id, label: team.name }))]);
const assignedAgent = computed(() => state.workspace.agents.find(agent => agent.id === ticket.value?.assignee_id));
const assignedTeam = computed(() => state.workspace.teams.find(team => team.id === ticket.value?.team_id));
async function saveProperty(key, value) {
    if (propertyDisabled.value) return;
    savingProperty.value = key;
    try { await update({ [key]: value }); } catch {} finally { savingProperty.value = ''; }
}
...
async function addTag() { const tag = newTag.value.trim(); if (tag && !propertyDisabled.value) { savingProperty.value = 'tags'; try { await update({ tags: [...new Set([...(ticket.value.tags || []), tag])] }, 'Tag added'); newTag.value = ''; } catch {} finally { savingProperty.value = ''; } } }
...
<main v-else class="ticket-page surface">
        ...
        <div ref="scroll" class="conversation-scroll">
            <div class="ticket-body-content">
                ...
                <article class="message-card" :class="{ outbound: message.kind === 'outbound', note: message.kind === 'note', automated: !!message.rule_name }">
                    ...
                    <div v-if="message.attachments?.length" class="message-files"><a v-for="(file, index) in message.attachments" :key="index" :href="file.preview_url || file.url" class="attachment-link" :download="!file.preview_url" :target="file.preview_url ? '_blank' : null" :rel="file.preview_url ? 'noopener noreferrer' : null" :title="file.preview_url ? 'View image in a new tab' : 'Download attachment'"><Icon :name="file.preview_url ? 'image' : 'file'" /><span>{{ file.name }}<small>{{ Math.ceil(file.size / 1024) }} KB</small></span><Icon :name="file.preview_url ? 'expand' : 'download'" :size="14" /></a></div>
        ...
        <div v-if="!ticket.merged_into_id" class="composer-wrap" :class="{ 'suggestions-open': showSuggestions }"><div class="ticket-body-content">
            <form class="composer" :class="{ 'private-composer': privateNote, 'suggestions-open': showSuggestions }" @submit.prevent="send">
                <div ref="replyEditorArea" class="reply-editor-area">
                    <button type="button" class="icon-button composer-expand" @click="expanded = !expanded" :aria-label="expanded ? 'Collapse reply editor' : 'Expand reply editor'" :aria-expanded="expanded"><Icon :name="expanded ? 'minimize' : 'expand'" :size="15" /></button>
    ...
    <aside v-if="details" class="ticket-inspector" aria-label="Ticket information">
        ...
        <fieldset class="inspector-fields" :disabled="Boolean(ticket.merged_into_id)">
            <InspectorSection title="Ticket info" class="inspector-section">
                <div class="inspector-field"><span>Status</span><InspectorSelect :model-value="ticket.status" :options="statusOptions" label="Ticket status" kind="status" :disabled="propertyDisabled" @update:model-value="saveProperty('status', $event)" /></div>
                <div class="inspector-field"><span>Priority</span><InspectorSelect :model-value="ticket.priority" :options="priorityOptions" label="Ticket priority" kind="priority" :disabled="propertyDisabled" @update:model-value="saveProperty('priority', $event)" /></div>
                <div class="inspector-field"><span>Created</span><span>{{ new Date(ticket.created_at).toLocaleDateString() }}</span></div>
                <div class="inspector-field"><span>Last activity</span><span>{{ new Date(ticket.last_activity_at || ticket.created_at).toLocaleDateString() }}</span></div>
                <div class="inspector-field"><span>Source</span><span class="inspector-source" aria-label="Ticket source"><Icon name="mail" :size="14" />{{ ticket.source }}</span></div>
                <p v-if="savingProperty" class="inspector-saving muted" role="status">Saving changes…</p>
            </InspectorSection>
            <InspectorSection title="Tags" class="inspector-section"><div v-if="ticket.tags?.length" class="inspector-tags"><span v-for="tag in ticket.tags" :key="tag" class="tag"><Icon name="tag" :size="12" />{{ tag }}<button type="button" @click="saveProperty('tags', ticket.tags.filter(t => t !== tag))" :disabled="propertyDisabled" :aria-label="'Remove tag ' + tag"><Icon name="x" :size="12" /></button></span></div><form class="add-tag" @submit.prevent="addTag"><Icon name="plus" :size="14" /><input v-model="newTag" placeholder="Add a tag…" aria-label="Add a tag" maxlength="60" :disabled="propertyDisabled" /><button type="submit" :disabled="!newTag.trim() || propertyDisabled">Add</button></form></InspectorSection>
            ...
            <InspectorSection title="Responsibility" class="inspector-section inspector-responsibility">
                <div class="assignment-block"><div class="inspector-block-heading"><span>Team</span></div><InspectorSelect :model-value="ticket.team_id ?? null" :options="teamOptions" label="Ticket team" kind="team" searchable :disabled="propertyDisabled" @update:model-value="saveProperty('team_id', $event)" /><p v-if="!assignedTeam" class="muted">Choose a team to own this conversation.</p></div>
                <div class="assignment-block"><div class="inspector-block-heading"><span>Agent</span><button v-if="ticket.assignee_id !== state.user.id" type="button" class="text-button" :disabled="propertyDisabled" @click="saveProperty('assignee_id', state.user.id)">Assign me</button><span v-else class="assigned-to-you">Assigned to you</span></div><InspectorSelect :model-value="ticket.assignee_id ?? null" :options="agentOptions" label="Ticket assignee" kind="agent" searchable :disabled="propertyDisabled" @update:model-value="saveProperty('assignee_id', $event)" /><p v-if="!assignedAgent" class="muted">No agent assigned to this ticket.</p></div>
            </InspectorSection>
            <InspectorSection title="Requester" class="inspector-section"><div class="inspector-block-heading"><span>Contact details</span><button type="button" class="text-button" @click="editDetails">Change</button></div><div class="requester-card"><span class="avatar">{{ initials(ticket.requester_name || ticket.requester_email) }}</span><div><strong>{{ ticket.requester_name || ticket.requester_email }}</strong><small v-if="ticket.requester_name">{{ ticket.requester_email }}</small></div></div><span v-if="ticket.company" class="muted">{{ ticket.company }}</span></InspectorSection>
            ...
            <InspectorSection title="People in the loop" class="inspector-section"><span v-if="!ticket.cc?.length" class="muted">No additional recipients</span><div v-for="email in ticket.cc" :key="email" class="cc-email"><Icon name="mail" :size="13" />{{ email }}</div><button type="button" @click="editDetails" class="text-button inspector-add-people" aria-label="Edit CC recipients"><Icon name="plus" :size="14" />Add or edit recipients</button></InspectorSection>
...
.ticket-page .ticket-inspector{width:320px;flex-basis:320px;max-width:100%}
.ticket-inspector>header{padding-inline:22px}.ticket-inspector>header h2{font-size:14px;font-weight:600}
.ticket-inspector .inspector-section{padding:20px 22px}.ticket-inspector .inspector-disclosure>summary h3{font-size:13px;font-weight:600}.ticket-inspector .inspector-disclosure>summary>svg{color:var(--muted)}
.ticket-inspector .inspector-field{font-size:12px;min-height:28px;margin-bottom:10px;gap:14px}.ticket-inspector .inspector-field>.inspector-select{max-width:175px}.inspector-source{display:inline-flex;align-items:center;gap:6px}
.inspector-block-heading{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px;font-size:12px;font-weight:500}.inspector-block-heading .text-button{font-size:11px;font-weight:400}
.assignment-block+.assignment-block{margin-top:20px}.assignment-block>.inspector-select{width:100%;min-height:38px}.assignment-block>p{font-size:11px;line-height:1.5;margin:8px 0 0}.assigned-to-you{color:var(--accent-ink);font-size:11px;font-weight:400}
.ticket-inspector .add-tag{border:1px dashed var(--line);border-radius:7px;padding:4px 8px;gap:7px}.ticket-inspector .add-tag:focus-within{border-style:solid;border-color:var(--accent)}.ticket-inspector .add-tag input{min-width:0;flex:1;padding:6px 0;font-size:12px}.ticket-inspector .add-tag button{font-size:11px;color:var(--accent-ink);padding:4px}.ticket-inspector .add-tag button:disabled{opacity:.4}.ticket-inspector .inspector-tags{margin-bottom:10px;gap:7px}.ticket-inspector .tag{gap:5px;padding:5px 7px;font-size:11px;max-width:100%;overflow-wrap:anywhere}.ticket-inspector .tag button{display:inline-flex;padding:2px;border-radius:3px}.ticket-inspector .tag button:hover{background:var(--selected)}
.ticket-inspector .requester-card{gap:10px;align-items:flex-start}.ticket-inspector .requester-card strong{font-size:12px;line-height:1.5;font-weight:500}.ticket-inspector .requester-card small{font-size:11px;line-height:1.5}.ticket-inspector .requester-card .avatar{width:34px;height:34px;border-radius:8px;flex-shrink:0}.ticket-inspector .requester-card+.muted{display:block;margin:8px 0 0 44px;font-size:11px}
.inspector-add-people{margin-top:12px;font-size:12px}.ticket-inspector .inspector-section-content>.muted{font-size:11px;line-height:1.6}.ticket-inspector .source-email{text-align:start;margin:8px 0 0;overflow-wrap:anywhere}.ticket-inspector .inspector-field select{border:1px solid var(--line);background:var(--surface);min-height:32px;font-size:12px;border-radius:7px}
.ticket-inspector .inspector-tools .ticket-tools{gap:7px}.ticket-inspector .inspector-tools .ticket-tools button{padding:10px 12px;border:1px solid var(--line);border-radius:7px;font-size:12px}.ticket-inspector .inspector-tools .ticket-tools button:hover{background:var(--soft)}
.ticket-inspector .inspector-translation{display:block}.inspector-saving{font-size:11px;margin:8px 0 0}
.ticket-inspector .inspector-section-content>.ticket-custom-fields{padding:0;border:0}
@media(max-width:1250px){.ticket-page .ticket-inspector{width:290px;flex-basis:290px}.ticket-inspector .inspector-section{padding-inline:18px}}
@media(max-width:1100px){.ticket-page .ticket-inspector{width:320px;flex-basis:320px}}
